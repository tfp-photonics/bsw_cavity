...
def main():
    fp = Path("20221030_opt_experiment.h5")
    mp.verbosity(0)

    runs = {}
    with h5py.File(fp, "r") as f:
        for gn, grp in f.items():
            runs[gn] = {k: v for k, v in grp.attrs.items()}
            runs[gn].update({k: np.array(v) for k, v in grp.items()})
    ...
    for name, run in tqdm(runs.items(), ncols=80, disable=not mp.am_master()):
        lcen = run["lcen"]
        d_wvg = run["d_wvg"]
        l_wvg = run["l_wvg"]
        radius = run["radius"]
        ...
        # "bulk" ldos
        cc = sources[0].center
        mg.update_weights(np.ones_like(design))
        ...
        box_x1 = sim.add_flux(
            ...
            fwidth,
            nfreqs,
            mp.FluxRegion(center=cc + mp.Vector3(0.5, 0), size=mp.Vector3(0, 1.0)),
        )
        box_x2 = sim.add_flux(
            fcen,
            fwidth,
        ...
        box_y1 = sim.add_flux(
            ...
            fwidth,
            nfreqs,
            mp.FluxRegion(center=cc + mp.Vector3(0, 0.5), size=mp.Vector3(1.0, 0)),
        )
        box_y2 = sim.add_flux(
            fcen,
            fwidth,
        ...
        dft = sim.add_dft_fields(
            ...
            fwidth,
            1,
            center=cc - mp.Vector3(0, 0.5),
            size=mp.Vector3(2 * radius, 2 * radius + 1),
        )
        ...
        run["permittivity"] = sim.get_array(
            mp.Dielectric,
            center=cc - mp.Vector3(0, 0.5),
            size=mp.Vector3(2 * radius, 2 * radius + 1),
        )
